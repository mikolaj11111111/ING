...
class ScrapeOpsFakeBrowserHeaderAgentMiddleware:
    ...
    def process_request(self, request, spider):        
        ...
        # Sprawdza, czy klucz istnieje w random_browser_header przed przypisaniem
        if 'accept-language' in random_browser_header:
            request.headers['accept-language'] = random_browser_header['accept-language']
        if 'sec-fetch-user' in random_browser_header:
            request.headers['sec-fetch-user'] = random_browser_header['sec-fetch-user']
        if 'sec-fetch-mod' in random_browser_header:
            request.headers['sec-fetch-mod'] = random_browser_header['sec-fetch-mod']
        if 'sec-fetch-site' in random_browser_header:
        ...
        if 'accept' in random_browser_header:
            request.headers['accept'] = random_browser_header['accept']
        if 'user-agent' in random_browser_header:
            request.headers['user-agent'] = random_browser_header['user-agent']
        if 'upgrade-insecure-requests' in random_browser_header:
